...
class GA_Solution:
    ...
    def __init__(self, i, population_size, num_generations):
    ...
    def GA_find_better_answer(self):
        if self.num_rows < 2:
            ...
            # 显示每一代的最佳适应度值和其他统计信息
            for g, avg, min_, max_ in zip(gen, fit_avg, fit_min, fit_max):
                print(f"Generation {g}: Average Fitness: {avg}, Min Fitness: {min_}, Max Fitness: {max_}")
...
def GA_find_better_answer_parallel(i):
    ga_solution = GA_Solution(i, population_size, num_generations)
    best_result, best_loss = ga_solution.GA_find_better_answer()
    return best_result, best_loss

if __name__ == "__main__":
    num_processes = min(53, cpu_count())  # 使用cpu核心数或者指定的最大进程数
    pool = Pool(num_processes)
    
    results = pool.map(GA_find_better_answer_parallel, range(53))
    pool.close()
    pool.join()
    ...
    Loss = []
    for i in range(53):
        best_result, best_loss = results[i]
